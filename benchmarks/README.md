 ...
 * run `m perf-setup` to build perf-setup.sh under $(OUT)
 * run `adb push $ANDROID_PRODUCT_OUT/obj/NATIVE_TESTS/perf-setup_intermediates/perf-setup.sh /data/local/tmp/perf-setup.sh` to device
 * run `adb shell chmod +x /data/local/tmp/perf-setup.sh`
