...
// (it doesn't have to be MTE capable), and then run a dummy executable using
// LD_LIBRARY_PATH and using the linker in interepreter mode (e.g.
// `LD_LIBRARY_PATH=/data/tmp/ /data/tmp/linker64 /data/tmp/my_dummy_binary`). A
// good heuristic is that the global variable is in a file that should be
// compiled with `-ffreestanding` (but there's GVs there that don't need this
// attribute).
