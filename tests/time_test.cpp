...
TEST(time, timer_create_EINVAL) {
  ...
  ASSERT_ERRNO(EINVAL);
  // We don't guarantee that the thread will be dead _before_ timer_create()
  // returns because that would require extra synchronization that's unnecessary
  // in the normal successful case. So give the thread time to die before
  // checking it's gone...
  sleep(1);
