...
TEST(stdlib, aligned_realloc_sweep) {
  SKIP_WITH_HWASAN;
  // Verify powers of 2 up to 2048 allocate, and verify that all other
  // alignment values between the powers of 2 fail.
  size_t last_align = 1;
  for (size_t align = 1; align <= 2048; align <<= 1) {
    // Try all of the non power of 2 values from the last until this value.
    for (size_t fail_align = last_align + 1; fail_align < align; fail_align++) {
      void* old_ptr = aligned_alloc(align, align);
      ASSERT_TRUE(old_ptr != nullptr);
      ASSERT_TRUE(aligned_realloc(old_ptr, fail_align, fail_align) == nullptr)
          << "Unexpected success at align " << fail_align;
      ASSERT_ERRNO(EINVAL) << "Unexpected errno at align " << fail_align;
    }
    void* old_ptr = aligned_alloc(align, align);
    ASSERT_TRUE(old_ptr != nullptr);
    void* ptr = aligned_realloc(old_ptr, align, 2 * align);
    ASSERT_TRUE(ptr != nullptr) << "Unexpected failure at align " << align;
    ASSERT_EQ(0U, reinterpret_cast<uintptr_t>(ptr) & (align - 1))
        << "Did not return a valid aligned ptr " << ptr << " expected alignment " << align;
    free(ptr);
...
TEST(stdlib, aligned_realloc_overflow) {
  SKIP_WITH_HWASAN;
  void* ptr = aligned_alloc(16, 16);
  ASSERT_TRUE(ptr != nullptr);
  ASSERT_TRUE(aligned_realloc(ptr, 16, SIZE_MAX) == nullptr);
}

TEST(stdlib, aligned_realloc_size_not_multiple_of_alignment) {
  SKIP_WITH_HWASAN;

  void* ptr = aligned_alloc(2, 2);
  ASSERT_TRUE(ptr != nullptr);
  ASSERT_TRUE(aligned_realloc(ptr, 2048, 1) == nullptr);
  ptr = aligned_alloc(2, 2);
  ASSERT_TRUE(ptr != nullptr);
  ASSERT_TRUE(aligned_realloc(ptr, 4, 3) == nullptr);
  ptr = aligned_alloc(2, 2);
  ASSERT_TRUE(ptr != nullptr);
  ASSERT_TRUE(aligned_realloc(ptr, 4, 7) == nullptr);
  ptr = aligned_alloc(2, 2);
  ASSERT_TRUE(ptr != nullptr);
  ASSERT_TRUE(aligned_realloc(ptr, 16, 8) == nullptr);
}
#endif
